...
この記事はアドベントカレンダー「Goで作るセキュリティ分析生成AIエージェント」の4日目です。今回は開発環境のセットアップとコードの概要について一通り解説し、明日からの開発が可能な状態にまでなるのが目的です。

# 開発環境の準備

すでにGo言語の開発環境およびGoogle CloudやClaude (Anthropic) サービスを利用しており自力で設定可能な方はセットアップに関して読む必要はありません。今回で準備が必要なのは以下のとおりなので、一通りそろえてもらったうえで次に進んでいただいて大丈夫です。
