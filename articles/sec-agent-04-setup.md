...
この記事はアドベントカレンダー「Goで作るセキュリティ分析生成AIエージェント」の4日目です。今回は開発環境のセットアップについて一通り解説し、明日からの開発が可能な状態にまでなるのが目的です。開発環境の設定方法および、開発で利用するためにこちらで事前実装したベースコードについて解説します。
...
ベースコードを見て気づいたかもしれませんが、現時点では `new` コマンドでアラートを登録してもタイトルは空のままです。まだLLMとの連携ができていないからです。でもこれで良いのです。ベースコードがあるからこそ、次のステップでGemini APIを組み込めば、一気にアラート要約機能が動き出します。
