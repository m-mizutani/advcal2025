...
ワークフロー機能は `pkg/workflow/` パッケージに実装します。UseCaseレイヤーに密着した形で実装する手もありますが、今回はわかりやすさを重視してパッケージとして分離しました。実装コード全体を掲載すると長くなるため、本記事では要所のみを抽出して解説します。全体のコードはリポジトリを参照してください。

以下の図は、ワークフロー全体のデータフローを示したものです。各フェーズでRegoポリシーの評価を実施し、その結果に応じて必要な処理や分岐を入れています。**Ingestフェーズ**は1回の入力から0個以上のアラートを生成し、**EnrichフェーズとTriageフェーズ**は生成された各アラートに対して個別に実行されます
...
`package` は名前空間を指定するキーワードで、今回は `ingest`, `enrich`, `triage` という3つのパッケージに分けています。これらのパッケージがそれぞれのフェーズで評価されることになります。ルール定義では、`if` ブロック内に記述した条件（AND条件として評価される）が満たされると、`contains` で指定された内容が `alert` というSet型の変数に格納されます。

`alert` セットには0個以上のデータを格納でき、ここに格納されたデータが次の段階（Enrichフェーズ）へ進むことになります。条件を満たさなかった場合は何も格納されず、そのアラートは棄却されます
...
Regoポリシーを評価するGoの実装を見ていきます。実装のポイントは3つあります。

まず、`PreparedEvalQuery`を使って初期化時にポリシーを事前コンパイルしておくことで、実行時のオーバーヘッドを削減します。次に、評価結果は`map[string]any`型として取得されるため、型アサーションを使ってGo構造体に変換する必要があります。最後に、`alert`セットが空の場合は棄却を意味し、これはエラーではなく正常な動作です
...
Enrichフェーズは、アラートに対する追加調査をLLMエージェントに実行させるフェーズです。Enrichフェーズの特徴は、**プロンプトの定義**と**プロンプトの実行**を分離している点です。
...
各プロンプトオブジェクトには3つのフィールドがあります。`id` フィールドは、あとからどのプロンプトの結果を利用するかを識別するためのユニークな識別子です。`content` フィールドには、LLMに発行する具体的なプロンプト内容を記述します。元アラートの情報はデフォルトでプロンプトに含まれるため、追加で必要な調査内容や観点を指示することになります。

`format` フィールドでは、応答形式を `text` または `json` で指定できます。`json` を指定すると、Goランタイム側でJSON文字列としてパースし、構造化データとして取得できます。これにより、後続のTriageフェーズでの解析が容易になります。一方、サマリや見解といった文章形式の応答が必要な場合は `text` を指定します
...
プロンプトは、他のプロンプトと同様にファイルをembedして `text/template` パッケージで構築すると、メンテナンス性が向上します。テンプレート内の `.PromptContent` にRegoで定義したタスク内容を差し込み、アラート情報を構造化して提供することで、LLMが必要な情報にアクセスできるようにします
...
ポリシーの書き方の基本はIngestやEnrichと同じですが、Triageフェーズで扱う `action`, `severity`, `note` はセット型ではなく文字列型である点に注意が必要です。これらの変数には原則として1つの値しか書き込めず、複数のルールで異なる値を代入しようとするとエラーになります。`default` キーワードで指定したデフォルト値は例外で、他のルールで上書きされない場合にのみ使用されます。もし複数のルールで評価した結果から最も重大度の高い値を選択したい場合は、セット型にしてGoのランタイム側で判定処理を実装するという手もあります。

Triageフェーズでは2つの入力データにアクセスできます。`input.alert` にはアラート自体の情報（title、description、attributes）が含まれており、`input.enrich` にはEnrichフェーズの実行結果が配列形式（`[]{ "id": "...", "result": "..." }`）で格納されています。Enrich結果の中身はテキストまたはJSON文字列なので、`contains` 関数で文字列検索したり、`json.unmarshal` でパースして構造化データとして扱ったりできます
...
Regoはファイル名やファイルパスに依存しない設計になっているため、指定したディレクトリ以下から全てのRegoファイルを読み込み、クエリだけを事前に各フェーズに振り分けます。`PrepareForEval()` メソッドで事前コンパイルすることで、実行時のオーバーヘッドを削減できます。ポリシーファイルが存在しない場合は `nil` を返しますが、これはエラーではなく正常な動作です。クエリは `data.<package名>` という形式で指定します（例：`data.ingest`、`data.enrich`、`data.triage`）
...
`Execute` 関数は、ワークフロー全体を統括する中核的な関数です。Ingestフェーズは1回だけ実行し、0個以上のアラートを生成します。EnrichフェーズとTriageフェーズは、生成された各アラートに対して独立して実行されるため、並列化が可能です。各フェーズの結果は最終的に `WorkflowResult` 構造体に集約されます
...
プロンプトに関しては、期待通りの動作を実現するためにしっかりと調整する必要があります。実際に運用してみると、LLMが思ったとおりに動いてくれないケースが多々あります。これはモデル自体の性能の問題もありますが、多くの場合はプロンプトに含まれるコンテキスト情報が不足していることが原因です。アラートの背景情報や期待する調査の観点を明確に記述することで、精度が向上します。
...
ポリシーのチューニング作業自体にも生成AIを活用できます。ポリシーのスキーマやルールを事前に生成AIに入力し、かつ具体的な事例を提示すると、適切なルール修正案を提案してくれます。ただし、Regoは比較的マイナーな言語であるため、完全に独自のDSL言語を使う場合に比べれば有利ですが、それでも生成AIの対応精度には限界がある点に注意が必要です
