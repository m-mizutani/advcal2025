...
warrenは実際に筆者の業務内でも利用しています。その実運用での効果や課題について紹介します。
...
本記事では、これまで実装してきたLLMエージェントをオンラインシステムに統合する際の考え方について、warrenを例に解説しました。

アラート管理とアラート分析を統合することで、LLMエージェントの能力を最大限に活用できます。分析の過程を記録し、それを記憶システムで活用したり、プロンプトエンジニアリングで組織固有の情報を注入したり、Function Callingでアラートへの操作を自動化したりといったことが可能になります。

アラート管理・分析システムには、アラートの受信、対応・分析、管理という3つの要件があります。特に重要なのは、使い慣れたツールで作業でき、関係者とシームレスにコミュニケーションをとれる仕組みです。warrenではSlackとWeb UIを組み合わせることで、インタラクティブな対話と体系的な管理の両方を実現しています。

実運用においても、ログ検索の自動化や非専門メンバーの分析業務サポートといった形で効果を発揮しています。もちろん改良の余地はまだまだありますが、LLMエージェントをセキュリティ分析業務に統合することで、大幅な省力化と業務の質の向上が期待できます。

明日は最終日として、このアドベントカレンダー全体の振り返りと、今後の展望について解説します。
