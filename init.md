# セキュリティアラート対応生成AIエージェント実装 - 25日間アドベントカレンダー
...
## Day 7: 会話と履歴の管理（12月7日 日曜日）
**slug: `sec-agent-07-conversation-history`**

- 生成AIにおける「会話」とは
- 会話履歴のデータ構造設計
- メッセージの追加と取得
- Cloud Storageでの永続化実装
- セッション管理：開始、継続、終了
- 履歴の検索と参照
- メタデータとコンテンツの分離戦略

## Day 8: Function Callingによる外部ツール連携（12月8日 月曜日）
**slug: `sec-agent-08-function-calling`**

- Function Callingとは：生成AIが外部機能を呼び出す仕組み
- アラート検索ツールの設計（field/operator/value パターン）
- ツール定義の実装：JSON Schemaでのパラメータ定義
- Function Callingフロー：リクエスト検出、パース、実行、結果返却
- ツール実行ループと最大呼び出し回数制限
- 実践：会話でアラートを検索

## Day 9: 脅威インテリジェンスツールの実装（12月9日 火曜日）
...
## Day 10: MCPで拡張可能なツール基盤（12月10日 水曜日）
...
## Day 11: プロンプトチューニングの基礎（12月11日 木曜日）
