...
このドキュメントは、2025年12月のアドベントカレンダー「[Goで作るセキュリティ分析LLMエージェント](https://adventar.org/calendars/11354)」の全体構成をまとめたものです。実際に執筆済みの記事(Day 1-9)の内容を反映し、今後の記事(Day 10-25)の方向性を示します。
...
## 第3週: 上級編 - ツール・エージェント、ワークフロー、Plan-Execute、記憶システム (Day 15-21)

### 🚧 Day 15: セキュリティ分析のためのツール・エージェント
**slug**: `sec-agent-15-tools-agents`

**内容**:
- ツール、エージェント、ワークフローの違い
...
**キーポイント**:
- ツール：特定機能を提供する単一目的コンポーネント
- エージェント：複数ツールを組み合わせて目的達成する自律的存在
- ワークフロー：決定的な手順を定義する処理系
- セキュリティ分析での適切な使い分け

---

### 🚧 Day 16: AIワークフロー (1) - 設計
**slug**: `sec-agent-16-workflow-design`

**内容**:
- AIワークフローとは：生成AI機能を組み込んだワークフローエンジン
- ワークフローエンジンの基本（処理、入出力、条件分岐の接続）
- AIの3つの役割：データ加工、条件分岐判定、エージェント処理
- セキュリティ分析で利用するワークフロー（SOARとの違い）
- SOAR is deadとSOAR+AIの可能性
- ワークフローが必要な理由：決定性のある処理、初期トリアージ
- ワークフローを独立実装する利点：ロジック平易化、ランタイム・ロジック分離
- ワークフローの設計方針：決定性と柔軟性のバランス、ステートレス vs ステートフル

**キーポイント**:
- AIワークフロー = ワークフローエンジン + 生成AI機能（n8n、Difyなど）
- 決定性のある処理はルールで、柔軟な判断は生成AIで
- エージェントは伴走する存在、ワークフローは決定的な処理系
- ランタイムとロジックの分離で運用性を向上

---

### 🚧 Day 17: AIワークフロー (2) - 実装
**slug**: `sec-agent-17-workflow-implementation`

**内容**:
- ワークフローDSLの設計：YAML/JSONベースの定義
- ワークフローランタイムの実装
- ステップの種類：データ加工、条件分岐、ツール実行、生成AI呼び出し
- アラート事前処理ワークフローの実装例
- 初期トリアージフローの設計と実装
- ワークフロー実行結果の記録とトレース
- エラーハンドリングとリトライ
...
**キーポイント**:
- YAMLによる宣言的なワークフロー定義
- ステップごとの入出力管理とコンテキスト受け渡し
- 生成AIとルールベース処理の組み合わせ
- ワークフロー実行の可観測性

---

### 🚧 Day 18: Plan & Execute (1) - 基礎と実装
**slug**: `sec-agent-18-plan-execute`
...
### 🚧 Day 19: Plan & Execute (2) - 動的再計画
**slug**: `sec-agent-19-plan-execute-replanning`
...
### 🚧 Day 20: Embeddingによる類似アラート検索
**slug**: `sec-agent-20-embedding-similarity`
...
### 🚧 Day 21: エージェントの記憶システム
**slug**: `sec-agent-21-memory-system`
...
### 🚧 Day 22: エラーハンドリングとリカバリー戦略
**slug**: `sec-agent-22-error-handling`
...
## 第4週: 実践編 - テスト、統合、展望 (Day 22-25)

### 🚧 Day 23: ツールの信頼性を高めるテスト戦略
**slug**: `sec-agent-23-testing`
...
### アーキテクチャパターン
- Day 14: サブエージェント
- Day 15: ツール・エージェント・ワークフローの違い
- Day 16-17: AIワークフロー（設計・実装）
- Day 18-19: Plan & Execute
- Day 22: エラーハンドリングとリカバリー
...
### Week 3（Day 15-21）の変更（2025-11-22更新）
- **新規**: Day 15「セキュリティ分析のためのツール・エージェント」
  - ツール、エージェント、ワークフローの概念整理と違い
- **変更**: Day 16-17 ← 旧Day 15-16「AIワークフロー (1)(2)」
- **変更**: Day 18-19 ← 旧Day 17-18「Plan & Execute (1)(2)」
- **変更**: Day 20 ← 旧Day 19「Embedding類似検索」
- **変更**: Day 21 ← 旧Day 20「エージェントの記憶システム」
- **変更**: Day 22 ← 旧Day 21「エラーハンドリングとリカバリー戦略」

### Week 4（Day 22-25）の変更（2025-11-22更新）
- **変更**: Day 23 ← 旧Day 22「ツールの信頼性を高めるテスト戦略」
- **削除**: 旧Day 23「エージェントの判断精度を測る (evaluation)」
  - 内容を他の記事に統合（Day 23テスト戦略など）
- **変更なし**: Day 24「WebUIとSlack連携の設計」
- **変更なし**: Day 25「システム総括とマルチエージェントの未来」

### 主要な狙い
1. Day 15でツール・エージェント・ワークフローの概念を整理してから各実装に進む流れ
2. AIエージェントの本質的トピック（記憶、サブエージェント、エラー処理）に焦点
3. Evaluation記事を削除し、テスト記事に統合することで重複を削減
4. 25記事構成を維持しつつ、より論理的な流れを実現
